...
mod app {
    ...
    use crate::{
        actuators::{
            ActuatorState, CommandOutcome, MAX_PULSE_MS, MIN_PULSE_MS, apply_action,
            handle_buzzer_command,
        },
        display::St7735Display,
        protocol::{self, LINE_BUFFER_CAPACITY, TELEMETRY_INTERVAL_MS},
        sensors::{Environment, bh1750::Bh1750, dht11::Dht11, soil::SoilSensor},
    };
                ...
                b'\n' => {
                    if !ctx.local.line_buf.is_empty() {
                        ...
                        if let Ok(frame) = protocol::parse_command_line(line) {
                            if frame.kind == "cmd" {
                                if let (Some(target), Some(action)) = (frame.target, frame.action) {
                                    let (result, message) = if target == "buzzer" {
                                        ...
                                        let outcome = (
                                            ...
                                            .lock(|actuators, buzzer_pin, pulse_ms| {
                                                handle_buzzer_command(
                                                    actuators, buzzer_pin, pulse_ms, action,
                                                    duration,
                                                )
                                            });
                                        ...
                                        match outcome {
                                            ...
                                            CommandOutcome::Error(msg) => ("error", Some(msg)),
                                        }
